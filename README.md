...
## Status
**Milestone 1 — transparent proxy + live hooks (no scaling yet).** The DLL loads,
forwards every real `d3d9.dll` export, hooks the device (Reset/Present/EndScene),
and logs device creation + frames. It should render **identically to stock** —
that's the point: verify the hook chain before touching geometry. The actual UI
scaling is gated on RenderDoc captures (see `research/`).
...
| `research/`           | Your RenderDoc / Ghidra notes |
