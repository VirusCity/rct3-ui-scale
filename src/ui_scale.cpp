...
void OnEndScene(IDirect3DDevice9* /*device*/) {
  // ========================================================================
  // Render-side scaling: EXPLORED AND ABANDONED (see research/ui_pass_findings.md)
  // ------------------------------------------------------------------------
  // The frame inspector showed the UI is drawn with fixed-function XYZRHW
  // (pre-transformed, baked-pixel) vertices, identified by VS=0 && XYZRHW. The
  // render-side approach (implemented in ScaleDrawIfUI / the draw detours)
  // rewrites those vertices in flight to enlarge the UI. Two unfixable caveats
  // killed it as the real solution:
  //
  //   1. Hit-testing is NOT fixed — the game still places buttons at the
  //      original tiny pixels, so clicks miss the visually-enlarged UI.
  //   2. A uniform vertex scale can't honour per-element edge anchoring; the
  //      hook only sees final pixels, not which element docks to which edge.
  //
  // The shipped fix is the source patch (source_patch.*): it shrinks RCT3's
  // GUI2 reference canvas so the game itself lays the UI out larger, keeping
  // both rendering AND the game's own hit-testing consistent. This EndScene
  // notification is left inert; the render-side experiment lives in the draw
  // detours and is disabled in normal use.
  // ========================================================================
}
