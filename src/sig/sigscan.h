// sig/sigscan.h — minimal array-of-bytes (AOB) signature scanner over a
// module's executable sections (ported from v1, live-verified there).
//
// In the hybrid design signatures are TIMING-ONLY: they locate the functions
// to hook (UI creators), never data. The one data-adjacent facility,
// SetDwordOperand, bakes a DISCOVERY-derived slot address into a store
// pattern — data locating timing, which is the allowed direction.
//
